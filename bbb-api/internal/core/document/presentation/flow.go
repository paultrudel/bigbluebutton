...
func NewPresentationFlow(client document.Client) pipeline.Flow[*document.Presentation, *document.Presentation] {
	transformToMeetingRunning := pipeline.NewStep[*document.Presentation, *meeting.MeetingRunningRequest]().Transform(&UploadToMeetingRunning{})
	...
	filterPresentation := pipeline.NewStep[*document.Presentation, *document.Presentation]().Filter(&UploadFilter{}).Transform(&NoOpTransformer{})
